...
public class LightweightTest extends TestHelper {
  protected static final String DB_PATH = "target/databases/lightweight-edges";
  ...
  public void testEdgeChunkIsLoadedFromCurrentTx() {
    ...
    database.transaction(() -> {
      commodore[0] = database.newVertex("Supplier").set("name", "Commodore").set("date", System.currentTimeMillis()).save()
          .getIdentity();
      c64[0] = database.newVertex("Good").set("name", "Commodore64").set("date", System.currentTimeMillis()).save().getIdentity();
    });

    database.transaction(() -> {
      commodore[0].asVertex().newLightEdge("SELLS", c64[0], true);

      final Vertex vic20 = database.newVertex("Good").set("name", "Vic20").set("date", System.currentTimeMillis()).save();

      commodore[0].asVertex(false).newLightEdge("SELLS", vic20, true).modify().set("date", System.currentTimeMillis()).save();

      Assertions.assertEquals(0, commodore[0].asVertex().countEdges(Vertex.DIRECTION.OUT, "SELLS"));
